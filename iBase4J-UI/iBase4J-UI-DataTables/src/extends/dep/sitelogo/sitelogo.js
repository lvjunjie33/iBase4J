  ...
  CropAvatar.prototype = {
    ...
    ajaxUpload: function () {
      var url = this.$avatarForm.attr('action'),
      	  formData = new FormData(this.$avatarForm[0]),
          result = this.$img.cropper("getCroppedCanvas"),
          _this = this;
      
      if(result) {
    	  var fileImg = result.toDataURL('image/jpg');
    	  formData.append("imgData", fileImg);
    ...
    submitDone: function (result) {
		if (result.code == 200) {
			if(window.submitDone) {
				window.submitDone(result);
			}
		} else {
			error(result.msg);
		}
